		...
		foreach ($entities as $index => $entity) {
			...
			if ($display_option == "slim") {
				...
				if ($index < $num_highlighted) {
					...
					if ($show_timestamp) {
// 						$text .= "<acronym title='" . date("r", $entity->time_created) . "'>" . substr(date("r", $entity->time_created),0,16) . "</acronym>";
					...
					$result .= "<div>";
					if ($show_timestamp) {
						$result .= elgg_view_friendly_time($entity->time_created);
						$result .= ' - ';
// 						$result .= "<acronym title='" . strftime("%c", $entity->time_created) . "'>" . strftime("%d %b", $entity->time_created) . "</acronym> - ";
					}
